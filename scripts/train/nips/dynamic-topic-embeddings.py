                    ...
                    for bs in [100]:
                        for i in range(87, 89):
                            data_dir = os.path.join(
                                data_path, "preprocessed", "nips", "language", str(i))
                            ...
                            ndt_inference_parameters.update({
                                ...
                                "gumbel": None})
                            ndt_inference_parameters.get(
                                "lr_scheduler").update({"counter": 5})

                            ndt_parameters.update(
                                {"number_of_topics": number_of_topics})
                            ndt_parameters.update({'experiment_name': f'nips-{number_of_topics}-split-{i}'})

                            ndt_parameters.get("theta_q_parameters").update(
                            ...
                            ndt_parameters.get("eta_q_parameters").update(
                            ...
                            try:
                                ...
                                model = DynamicTopicEmbeddings(
                                    data_loader=data_loader, **ndt_parameters)
